...
export const ClientForm: React.FC<ClientFormProps> = ({ onAddClient, onUpdateClient, clientToEdit, onCancelEdit }) => {
  ...
  const labelClass = "block text-zinc-400 text-xs font-bold mb-1.5 ml-1 uppercase tracking-wider";
  // Compact Input Class - Reduced Height and Font
  const getInputClass = (fieldName: string) => `
    w-full bg-zinc-950/50 text-zinc-100 text-sm rounded-lg border py-2 pl-9 pr-3 placeholder-zinc-700 
  ...
  return (
    ...
    <div className="bg-zinc-900 p-3 md:p-4 rounded-2xl border border-zinc-800 shadow-xl shadow-black/20 relative h-full flex flex-col">
      {/* Accent Bar */}
      <div className={`absolute top-0 left-0 w-full h-1.5 rounded-t-2xl ${clientToEdit ? 'bg-amber-600' : 'bg-primary-600'}`}></div>
      
      {/* Form Header - Compact */}
      <div className="flex flex-col justify-between mb-4 pt-3 gap-3">
        <div className="flex items-center gap-3">
          <div className={`p-2 rounded-lg border ${clientToEdit ? 'bg-amber-500/10 border-amber-500/20' : 'bg-primary-500/10 border-primary-500/20'}`}>
            {clientToEdit ? <Edit2 className="w-5 h-5 text-amber-500" /> : <User className="w-5 h-5 text-primary-500" />}
          </div>
          <div>
            <h2 className="text-lg font-bold text-zinc-100 leading-tight">
              {clientToEdit ? 'Editar Cliente' : 'Novo Cliente'}
            </h2>
            <p className="text-zinc-500 text-xs">Preencha os dados abaixo</p>
          </div>
        </div>
        {clientToEdit && (
          <span className="self-start text-[10px] font-bold bg-amber-500/10 text-amber-500 px-2 py-1 rounded border border-amber-500/20 uppercase tracking-wide">
            Editando
          </span>
        )}
      </div>
      ...
      <div className="flex-1 grid grid-cols-1 md:grid-cols-12 gap-x-3 gap-y-4 content-start relative">
        ...
        <div className="col-span-1 md:col-span-6 group relative">
          ...
          <div className="relative">
            ...
            />
            <div className="absolute right-3 top-1/2 transform -translate-y-1/2 text-zinc-600 pointer-events-none">
              <ChevronDown size={14} />
            </div>
          </div>
          ...
          {showCountryList && (
            <div className="absolute top-[calc(100%+2px)] left-0 w-full bg-zinc-900 border border-zinc-700 rounded-lg shadow-2xl shadow-black/80 z-50 max-h-48 overflow-y-auto custom-scrollbar">
                ...
                <ul>
                  {filteredCountries.map((c) => (
                    <li 
                      ...
                      className="flex items-center gap-2 px-3 py-2 hover:bg-zinc-800 cursor-pointer transition-colors border-b border-zinc-800/50 last:border-0"
                    >
        ...
        <div className="col-span-1 md:col-span-6 flex flex-col justify-end">
           ...
           <div 
            className="w-full h-10 flex items-center justify-between px-3 border border-zinc-800 bg-zinc-950/50 rounded-lg hover:bg-zinc-800/50 hover:border-zinc-700 transition-all cursor-pointer select-none group active:scale-98" 
          ...
          >
            <span className={`text-xs font-bold transition-colors ${formData.isPaid ? "text-green-400" : "text-zinc-400 group-hover:text-zinc-300"}`}>
              {formData.isPaid ? "Pago" : "Pendente"}
            </span>
            <div className={`w-5 h-5 rounded flex items-center justify-center transition-all border ${formData.isPaid ? 'bg-green-600 border-green-600' : 'border-zinc-600 bg-zinc-900'}`}>
        ...
        <div className="col-span-1 md:col-span-12 flex gap-3 mt-4">
          <button 
            ...
            className={`flex-1 flex items-center justify-center gap-2 text-white h-11 px-2 rounded-lg text-sm font-bold shadow-lg transition-all hover:brightness-110 active:scale-95 ${clientToEdit ? 'bg-amber-600 shadow-amber-900/20' : 'bg-primary-600 shadow-primary-900/20'}`}
          >
          ...
          <button 
            onClick={handleClear}
            className="flex-1 flex items-center justify-center gap-2 bg-zinc-800 border border-zinc-700 text-zinc-300 hover:bg-zinc-700 hover:text-white h-11 px-2 rounded-lg text-sm font-medium transition-colors"
          >
